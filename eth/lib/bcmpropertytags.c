			       ...
			       void *pTag, unsigned nTagSize, unsigned  nRequestParmSize)
{
	...
	UK_ASSERT (pTag != 0);
	UK_ASSERT (nTagSize >= sizeof (TPropertyTagSimple));
	unsigned nBufferSize = sizeof (TPropertyBuffer) + nTagSize + sizeof (u32);
	UK_ASSERT ((nBufferSize & 3) == 0);
	...
	*pEndTag = PROPTAG_END;
	...
	clean_and_invalidate_dcache_range(pBuffer, nBufferSize);
	DataSyncBarrier ();
	...
	invalidate_dcache_range(pBuffer, nBufferSize);
	DataSyncBarrier ();
