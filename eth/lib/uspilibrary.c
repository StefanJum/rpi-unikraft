...
{
	LogWrite (LOG_DEBUG, "Initializing " USPI_NAME " " USPI_VERSION_STRING);

	UK_ASSERT (s_pLibrary == 0);
	...
	{
		LogWrite (LOG_ERROR, "Cannot initialize USB host controller interface");
	...
	LogWrite (LOG_DEBUG, USPI_NAME " successfully initialized");
...
{
	UK_ASSERT (s_pLibrary != 0);
	UK_ASSERT (s_pLibrary->pUMouse1 != 0);
