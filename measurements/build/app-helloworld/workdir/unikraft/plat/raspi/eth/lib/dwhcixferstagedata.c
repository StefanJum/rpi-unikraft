...
void DWHCITransferStageData (TDWHCITransferStageData *pThis, unsigned nChannel, TUSBRequest *pURB, boolean bIn, boolean bStatusStage)
{
	...
	UK_ASSERT (pThis->m_pURB != 0);

	pThis->m_pEndpoint = USBRequestGetEndpoint (pURB);
	UK_ASSERT (pThis->m_pEndpoint != 0);
	...
	UK_ASSERT (pThis->m_pBufferPointer != 0);
	UK_ASSERT (((uintptr) pThis->m_pBufferPointer & 3) == 0);
...
void DWHCITransferStageDataTransactionComplete (TDWHCITransferStageData *pThis, u32 nStatus, u32 nPacketsLeft, u32 nBytesLeft)
{
	...
	UK_ASSERT (nPacketsTransfered <= pThis->m_nPackets);
	...
	{
		UK_ASSERT (pThis->m_nTotalBytesTransfered <= pThis->m_nTransferSize);
...
{
	UK_ASSERT (pThis != 0);
	UK_ASSERT (pThis->m_bSplitTransaction);
...
{
	UK_ASSERT (pThis != 0);
	UK_ASSERT (pThis->m_nTransactionStatus != 0);
