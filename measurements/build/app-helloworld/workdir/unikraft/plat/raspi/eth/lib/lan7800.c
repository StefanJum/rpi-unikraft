...
boolean LAN7800DeviceConfigure (TUSBFunction *pUSBFunction)
{
	...
	while ((pEndpointDesc = (TUSBEndpointDescriptor *) USBFunctionGetDescriptor (&pThis->m_USBFunction, DESCRIPTOR_ENDPOINT)) != 0)
	{
		if ((pEndpointDesc->bmAttributes & 0x3F) == 0x02)		// Bulk
		{
			if ((pEndpointDesc->bEndpointAddress & 0x80) == 0x80)	// Input
			{
				...
				UK_ASSERT (pThis->m_pEndpointBulkIn);
			...
			else							// Output
			{
				...
				UK_ASSERT (pThis->m_pEndpointBulkOut);
	...
	{
		LogWrite (LOG_ERROR, "Invalid chip ID (0x%X)",
	...
	{
		LogWrite (LOG_ERROR, "HW reset failed");
	...
	{
		LogWrite (LOG_ERROR, "Cannot init MAC address");
	...
	{
		LogWrite (LOG_ERROR, "PHY reset failed");
	...
	{
		LogWrite (LOG_ERROR, "Cannot init PHY");
...
boolean LAN7800DeviceSendFrame (TLAN7800Device *pThis, const void *pBuffer, unsigned nLength)
{
	...
	*(u32 *) &pThis->m_pTxBuffer[4] = 0;
	...
	UK_ASSERT (pThis->m_pEndpointBulkOut != 0);
...
boolean LAN7800DeviceReceiveFrame (TLAN7800Device *pThis, void *pBuffer, unsigned *pResultLength)
{
	...
	UK_ASSERT (pThis->m_pEndpointBulkIn != 0);
	UK_ASSERT (pBuffer != 0);
	TUSBRequest URB;
	...
	{
		LogWrite (LOG_WARNING, "RX error (status 0x%X)", nRxStatus);
	...
	UK_ASSERT (nFrameLength == nResultLength-RX_HEADER_SIZE);
	UK_ASSERT (nFrameLength > 4);
	if (nFrameLength <= 4)
	{
		_USBRequest (&URB);
	...
	//LogWrite (LOG_DEBUG, "Frame received (status 0x%X)", nRxStatus);

	memcpy (pBuffer, (u8 *) pBuffer + RX_HEADER_SIZE, nFrameLength); // overwrite RX command A..C

	UK_ASSERT (pResultLength != 0);
...
boolean LAN7800DeviceInitMACAddress (TLAN7800Device *pThis)
{
	...
	LogWrite (LOG_DEBUG, "MAC address is %s", StringGet (&MACString));
...
boolean LAN7800DeviceWriteReg (TLAN7800Device *pThis, u32 nIndex, u32 nValue)
{
	...
	{
		LogWrite (LOG_WARNING, "Cannot write register 0x%X", nIndex);
...
boolean LAN7800DeviceReadReg (TLAN7800Device *pThis, u32 nIndex, u32 *pValue)
{
	...
	{
		LogWrite (LOG_WARNING, "Cannot read register 0x%X", nIndex);
