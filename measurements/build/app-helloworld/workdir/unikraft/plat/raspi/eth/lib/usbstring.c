...
boolean USBStringGetFromDescriptor (TUSBString *pThis, u8 ucID, u16 usLanguageID)
{
	...
	UK_ASSERT (pThis->m_pUSBString->bLength >= 2);
	UK_ASSERT ((pThis->m_pUSBString->bLength & 1) == 0);
	size_t nLength = (pThis->m_pUSBString->bLength-2) / 2;

	UK_ASSERT (nLength <= (255-2) / 2);
...
u16 USBStringGetLanguageID (TUSBString *pThis)
{
	...
	UK_ASSERT (pLanguageIDs->bLength >= 4);
	UK_ASSERT ((pLanguageIDs->bLength & 1) == 0);
